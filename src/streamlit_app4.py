...
from rag_chain import get_memory, crear_rag_chain,reformular_respuesta_local

from db_logger import buscar_en_base_datos
        ...
        with st.chat_message("assistant"):
            with st.spinner("Pensando..."):
                try:
                    ...
                    # 1️⃣ Busca en la base de datos
                    respuesta_bd = buscar_en_base_datos(user_text, db_path=os.path.abspath("./Code/rag_respuestas.db"))
                    if respuesta_bd:
                        # 2️⃣ Reformula la respuesta encontrada
                        st.write("🔄 Reformulando respuesta desde la base de datos...")
                        answer = reformular_respuesta_local(respuesta_bd, role)
                        docs = [Document(
                            page_content=respuesta_bd,
                            metadata={"source": "📂 Base de datos FAQ"}
                        )]
                    else:
                        # 3️⃣ Si no hay coincidencia, usar RAG normal
